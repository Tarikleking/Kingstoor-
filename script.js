...
// Email support modal — opens like the Policies window instead of scrolling to a section.
const supportEmailModal=document.getElementById('supportEmail');
const openSupportEmail=()=>{
  if(!supportEmailModal) return;
  supportEmailModal.classList.add('show');
  supportEmailModal.setAttribute('aria-hidden','false');
  document.body.classList.add('modalOpen');
};
const closeSupportEmail=()=>{
  if(!supportEmailModal) return;
  supportEmailModal.classList.remove('show');
  supportEmailModal.setAttribute('aria-hidden','true');
  document.body.classList.remove('modalOpen');
};
document.querySelectorAll('a[href="#supportEmail"]').forEach(a=>a.addEventListener('click',e=>{
  e.preventDefault();
  openSupportEmail();
  history.replaceState(null,'','#supportEmail');
}));
supportEmailModal?.querySelectorAll('[data-support-email-close]').forEach(el=>el.addEventListener('click',closeSupportEmail));
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeSupportEmail();closePolicies();}});
...
if(supportEmailForm){
  supportEmailForm.addEventListener('submit',async e=>{
    ...
    if(!submit||!supportEmailResult)return;
    const original=submit.innerHTML; submit.disabled=true; submit.innerHTML='جارٍ الإرسال...';
    supportEmailResult.className='supportEmailResult'; supportEmailResult.textContent='';
    try{
      const response=await fetch('https://formsubmit.co/ajax/support@kingstoor.com',{method:'POST',headers:{Accept:'application/json'},body:new FormData(supportEmailForm)});
      const data=await response.json().catch(()=>({}));
      if(!response.ok || data.success===false) throw new Error('send_failed');
      supportEmailResult.classList.add('success'); supportEmailResult.textContent='تم إرسال رسالتك بنجاح ✅'; supportEmailForm.reset();
    }catch(err){
      supportEmailResult.classList.add('error'); supportEmailResult.textContent='تعذر إرسال الرسالة حالياً. حاول مرة أخرى أو استخدم WhatsApp.';
